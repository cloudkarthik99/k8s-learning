...
kubeadm init --ignore-preflight-errors=NumCPU,Mem > /tmp/restult.out
...
aws s3 cp /tmp/join_command.sh s3://k8s-cluster-config-bucket-karthik;
...
aws s3 cp /etc/kubernetes/admin.conf s3://k8s-cluster-config-bucket-karthik
...
# sleep 5
# kubectl --kubeconfig /root/.kube/config create -f /root/calico.yaml
